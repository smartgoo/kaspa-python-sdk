...
The signature returned by `sign_message` is a hex-encoded Schnorr signature. It can be:
...
!!! warning "Important Security Notes"
    ...
    1. **Never sign arbitrary data** - Only sign messages you understand
    2. **Include context** - Add timestamps, addresses, or purposes to prevent replay attacks
    3. **Verify before trusting** - Always verify signatures before taking action
    4. **Protect private keys** - Signing doesn't expose your private key, but protect it anyway

## Error Handling

```python
from kaspa import sign_message, verify_message, PrivateKey, PublicKey
...
1. **Include timestamps** to prevent signature reuse
2. **Include purpose** in the message (e.g., "login", "transfer approval")
3. **Use deterministic signing** only when needed (testing, compatibility)
4. **Validate input** before signing or verifying
5. **Log verification attempts** for audit trails

## See Also

- [API Reference: Keys](../reference/index.md)
